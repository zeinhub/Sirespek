...
$no_urut = 0;
$search = $_POST['cari'];
$query = "SELECT * FROM transaksi WHERE id='$search'";
    ...
    <div style="padding-top: 20px;" class="container">
        <div class="row">
            <div class="col">
                ...
                <?php
                $status = $data['status'];
                if ($status == 'Validasi Pembayaran') { ?>
                    <h6 class="disabled btn btn-warning"><?php echo $data['status'] ?></h6>
                <?php } else if ($status == 'Menunggu') { ?>
                    <h6 class="disabled btn btn-info"><?php echo $data['status'] ?></h6>
                <?php } else if ($status == 'Dalam Antrian') { ?>
                    <h6 class="disabled btn btn-secondary"><?php echo $data['status'] ?></h6>
                <?php } else if ($status == 'Ditangani') { ?>
                    <h6 class="disabled btn btn-danger"><?php echo $data['status'] ?></h6>
                <?php } else if ($status == 'Selesai') { ?>
                    <h6 class="disabled btn btn-primary"><?php echo $data['status'] ?></h6>
                <?php } else {
                    ($status == 'Diambil')  ?>
                    <h6 class="disabled btn btn-success"><?php echo $data['status'] ?></h6>
                <?php }
                ?>
            </div>
